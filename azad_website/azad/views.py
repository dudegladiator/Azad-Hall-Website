...
def import_from_excel(request):
    if request.method == 'POST':
        ...
        for row in ws.iter_rows(values_only=True):
            roll_no, name, email, number = row
            azad_boarder = azad_boarders(name=name, roll_no=roll_no, emails=email, contact=number, books=0)
...
def importBooksFromExcel(request):
    if request.method == 'POST':
        ...
        for row in ws.iter_rows(values_only=True):
            book1 = book(title=row[0], author=row[1], department=row[2], shelf=row[3], quantity=row[4], available=row[4])
